...
function mn_decode(str, wordset_name) {
    ...
    var wlist = str.split(' ');
    var checksum_word = '';
    if (wlist.length < 12) throw "Invalid number of words";
    if ((wordset.prefix_len === 0 && (wlist.length % 3 !== 0)) ||
        (wordset.prefix_len > 0 && (wlist.length % 3 === 2))) throw "invalid number of words";
    if (wordset.prefix_len > 0 && (wlist.length % 3 === 0)) throw "missing checksum";
    if (wordset.prefix_len > 0) {
    ...
    for (var i = 0; i < wlist.length; i += 3) {
        ...
        if (x % n != w1) throw 'Failed to decode mnemonic';
...
function mn_random(bits) {
    'use strict';
    if (bits % 32 !== 0) throw "Invalid number of bits: " + bits;
    var array = new Uint32Array(bits / 32);
    if (!window.crypto) throw "Crypto api not supported!";
    var i = 0;
    ...
    if (arr_is_zero()) {
        throw "Failed to generate crypto random!";
    }
    // Convert to hex
    var out = '';
    for (var j = 0; j < bits / 32; ++j) {
...
var mn_words = {
    ...
    'english': {
        ...
        words: [
            ...
            "injury", "inkling", "incline", "inmate", "innocent", "inorganic", "input", "inquest",
            ...
            "ouch", "ought", "ounce", "launchpad", "oust", "outbreak", "oval", "oven",
